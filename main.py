...
async def generate_ai_data(image_url: str):
    try:
        # Check if it's a local file from your uploads folder
        if image_url.startswith("uploads") or os.path.exists(image_url):
            img = Image.open(image_url).convert("RGB")
        else:
            # It's a web URL (Pexels), fetch it normally
            async with httpx.AsyncClient() as client:
                response = await client.get(image_url, timeout=10.0)
                img = Image.open(BytesIO(response.content)).convert("RGB")

        # AI processing remains exactly the same
        ...
        print(f"AI Error: {e}") # Helpful for debugging in Pycharm
